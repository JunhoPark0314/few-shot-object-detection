...
from fsdet.utils.events import FasterRCNNWriter
import torch
...
from fsdet.engine.hooks import EvalHookFsdet, MemoryBankHook
from fsdet.evaluation import (
	DatasetEvaluator,
	inference_on_dataset,
	condition_on_dataset,
	print_csv_format,
	verify_results,
	meta_inference_on_dataset
)
from fsdet.modeling import build_model, build_meta_memory
from detectron2.data import (
	MetadataCatalog,
	build_detection_test_loader,
	build_detection_train_loader,
)
from fsdet.data import build_detection_condition_loader
...
class DefaultBankTrainer(BankTrainer):
	"""
	A trainer with default training logic. Compared to `SimpleTrainer`, it
	contains the following logic in addition:
	...
	def __init__(self, cfg):
		...
		ema_model.prepare_feature = True
		optimizer = self.build_optimizer(cfg, model)
		data_loader = self.build_train_loader(cfg)
		...
		memory = self.build_memory(cfg, mapping, keepclass, backbone_dim)
		...
		super().__init__(model, ema_model, memory, data_loader, optimizer, cfg.SOLVER.EMA_M)
		...
		self.checkpointer = DetectionCheckpointer(
			...
			optimizer=optimizer,
			scheduler=self.scheduler,
			ema_model=ema_model.module if comm.get_world_size() > 1 else ema_model
		)
		self.start_iter = 0
		self.max_iter = cfg.SOLVER.MAX_ITER
		self.cfg = cfg
	...
	def build_writers(self):
		...
		return [
			...
			JSONWriter(os.path.join(self.cfg.OUTPUT_DIR, "metrics.json")),
			TensorboardXWriter(self.cfg.OUTPUT_DIR),
			FasterRCNNWriter(self.max_iter),
		]
	...
	@classmethod
	def build_condition_loader(cls, cfg, dataset_name):

		return build_detection_condition_loader(cfg, dataset_name)
	...
	@classmethod
	def build_memory(cls, cfg, cls_mapping, keepclass, memory_dim):
		"""
		Returns:
			DatasetEvaluator
		...
		It is not implemented by default.
		"""
		return build_meta_memory(cfg, cls_mapping, keepclass, memory_dim)
