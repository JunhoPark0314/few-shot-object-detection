...
class BankRPN(nn.Module):
    ...
    def _subsample_labels(self, label):
        ...
        assert torch.logical_and(pos_idx < len(label), pos_idx >=0).all().item()
        assert torch.logical_and(neg_idx < len(label), neg_idx >=0).all().item()
    ...
    def label_and_sample_anchors(
        ...
        for image_size_i, gt_boxes_i, gt_classes_i in zip(image_sizes, gt_boxes, gt_classes):
            ...
            matched_gt_classes.append(gt_class_i)
    ...
    def forward(
        ...
        if prepare_feature:
            # TODO_P: use self.boxtransform to get gt_box_deltas and pass it to gt_feature

            anchors_tensor = type(anchors[0]).cat(anchors).tensor  # (R, 4)
            gt_anchor_deltas = [self.box2box_transform.get_deltas(anchors_tensor, k) for k in gt_boxes]
            gt_anchor_deltas = torch.stack(gt_anchor_deltas)
            feature_dict = self.prepare_gt_feature(features, gt_anchor_deltas, gt_labels, gt_class)
        else:
            feature_dict = {}

        return proposals, losses, feature_dict
    
    def prepare_gt_feature(
        self,
        features,
        gt_anchor_deltas,
        gt_labels,
        ...
        """
        prepare ground truth part feature, labels, and their relational positional encoding
        """
        with torch.no_grad():
            flatten_feature = [
                # (N, A*B, Hi, Wi) -> (N, A, B, Hi, Wi) -> (N, Hi, Wi, A, B) -> (N, Hi*Wi*A, B)
            ...
            flatten_scale = [
                torch.ones_like(x[...,0]) * i
                for i, x in enumerate(flatten_feature)
            ]
            flatten_feature = torch.cat(flatten_feature, dim=1)
            flatten_scale = torch.cat(flatten_scale, dim=1)

            num_anchors = gt_anchor_deltas.shape[1] // flatten_feature.shape[1]
            flatten_feature = flatten_feature.unsqueeze(-2).repeat(1,1,num_anchors,1).flatten(1, -2)
            flatten_scale = flatten_scale.unsqueeze(-1).repeat(1,1,num_anchors).flatten(1, -1)
            ...
            gt_feature = flatten_feature[pos_mask]
            gt_deltas = gt_anchor_deltas[pos_mask]
            gt_scale = flatten_scale[pos_mask]
