...
import numpy as np
import torch
from fvcore.nn import smooth_l1_loss, sigmoid_focal_loss_jit
...
from fsdet.layers import DynamicCondLinear
...
class BankRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        """
        num_instances = self.gt_classes.numel()
        max_score, pred_classes = self.pred_class_logits.max(dim=1)
        pred_classes += 1
        bg_ind = max_score.sigmoid() < self.roi_threshold
        bg_class = 0
        ...
        num_false_negative = (
            (fg_pred_classes == bg_class).nonzero().numel()
        )
        num_accurate = (pred_classes == self.gt_classes).nonzero().numel()
        fg_num_accurate = (fg_pred_classes == fg_gt_classes).nonzero().numel()
    ...
    def smooth_l1_loss(self):
        ...
        loss_box_reg = smooth_l1_loss(
            self.pred_proposal_deltas[fg_inds],
            gt_proposal_deltas[fg_inds],
            self.smooth_l1_beta,
            reduction="sum",
        )
